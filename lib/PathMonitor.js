...
class PathMonitor {
    ...
    informClients(assetPath, unlinkedAssetPath) {
        ...
        for (const client of clientsToInform) {
            const assetPath = this.clientAssets.get(client);
            client.processEvent({
                type: "reload",
                args: { assetPath }
            });
    ...
    async _linkClientHandleOrphans(client, orphans) {
        ...
        for (const orphanAssetPath of orphans) {
            const leafAssetPath = await this.resolveLeafForAssetPath(
                orphanAssetPath
            );
            if (leafAssetPath === clientAssetPath) {
    ...
    loadHtmlAssetAndPopulate(assetPath) {
        const promise = this._loadAsset(assetPath, async pathAsset => {
            if (pathAsset.dirty) {
                await pathAsset.asset.unload();
                await pathAsset.asset.load();
            }

            try {
                await this.assetGraph.populate({
                    from: pathAsset.asset,
                    followRelations: {
                        to: {
    ...
    loadJsAssetAndPopulate(assetPath) {
        const promise = this._loadAsset(assetPath, async pathAsset => {
            ...
            const importNodes = asset.parseTree.body
                .filter(node => node.type === "ImportDeclaration")
                .map(node => node.source.value)
                .filter(maybeNodeImport => /^[a-z@]/.test(maybeNodeImport));
            if (importNodes.length > 0) {
                const servePathRequire = Module.createRequire(this.servePath);
                for (const importRef of importNodes) {
                    const modulePath = servePathRequire.resolve(importRef);
                    const {
                        moduleName,
                        moduleFilePath
    ...
    async notifyClientForFsPath(fsPath) {
        ...
        const leafAssetPath = await this.resolveLeafForAssetPath(assetPath);
        this.informClients(leafAssetPath, assetPath);
    }
    ...
    async notifyClientForFsPathDelete(fsPath) {
        ...
        if (this.alwaysUpdateClients) {
            ...
            this.informClients(leafAssetPath, null);
        }
    }
