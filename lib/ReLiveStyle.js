...
class ReLiveStyle {
    ...
    addClient(client, assetPath) {
        ...
        const orphans = Object.keys(this.orphanByAssetPath);
        if (orphans.length > 0) {
            ...
            // it to be triggered.
            this._addClientHandleOrphans(client, orphans);
        }
    }

    async _addClientHandleOrphans(client, orphans) {
        ...
        for (const assetPath of orphans) {
            ...
            if (clientsToInform.includes(client)) {
                // The client being added was one affected by
                // the orphaned asset. Since both the asset
                // and its parent are now loaded we can safely
