...
module.exports = function(options) {
    ...
    function middleware(req, res, next) {
        ...
        if (
            ...
            isFile(path.join(options.servePath, `${assetPath.slice(1)}.html`))
        ) {
            // match the behaviour of serve-static
            assetPath = `/${assetPath.slice(1)}.html`;
        }

        const relevance = determineRelevance(assetPath);
