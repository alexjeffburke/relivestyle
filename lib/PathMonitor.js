...
class PathMonitor {
    ...
    loadAsset(assetPath, assetFn) {
        ...
        } else if (this.loadedByAssetPath[assetPath]) {
            return Promise.resolve(this.loadedByAssetPath[assetPath]);
        }

        this.promiseByAssetPath[assetPath] = this._loadAsset(
            assetPath,
            assetFn
        );
        return this.promiseByAssetPath[assetPath];
    ...
    async _loadAsset(assetPath, assetFn) {
        try {
            const [fromAsset] = await this.assetGraph.loadAssets(assetPath);
            if (typeof assetFn === "function") {
                await assetFn(fromAsset);
            }
            this.loadedByAssetPath[assetPath] = fromAsset;
            return fromAsset;
        } catch {
            // ignore
        } finally {
    ...
    loadHtmlAssetAndPopulate(assetPath) {
        return this.loadAsset(assetPath, fromAsset =>
            this.assetGraph.populate({
                from: { url: this.makeGraphUrlFromAssetPath(assetPath) },
                followRelations: {
                    to: {
